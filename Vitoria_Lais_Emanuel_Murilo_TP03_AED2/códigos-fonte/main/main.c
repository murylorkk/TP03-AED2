...
int main()
{
    ...
    /* ======================================================
       questão 2: busca usando hashing para o atributo chave;
       ====================================================== */
    ...
    /* =====================================================================
           NOTA:
           Como a busca O(1) na RAM ocorre na escala de nanossegundos, testamos
           colocar a busca num laço de 1.000 repetições para forçar a medição:
           printf("=== Estrategia 2: Busca via Hash (chave = matricula) ===\n");
            printf("%-5s %-10s %-15s %s\n", "#", "Chave", "Tempo (s)", "Nome encontrado");
            printf("----------------------------------------------------\n");
            ...
            for (int i = 0; i < NUM_BUSCAS; i++) {
                ...
                int num_reg;
                int achou = 0; // Inicializamos com 0 por segurança

                // Faz a mesma busca 1000 vezes só para o cronômetro conseguir "sentir" o tempo passar
                for(int repeticao = 0; repeticao < 1000; repeticao++) {
                    num_reg = hash_buscar(&tabela_hash, chaves_teste[i]);
                    achou = (num_reg >= 0) && ler_registro_disco(arq, num_reg, &reg_temp);
                }

                clock_t fim_busca = clock();

                // Agora divide o dt por 1000.0 para saber o tempo real de UMA busca
                double dt = ((double)(fim_busca - ini_busca) / CLOCKS_PER_SEC) / 1000.0;
                soma_tempo_q2 += dt;
           ...
           Optamos por não manter o laço ativo na versão final pois ele introduz
           o 'Efeito Cache' (o processador passa a ler do Cache L1 em vez da RAM),
           o que gera um tempo artificialmente otimizado.
           ===================================================================== */
    ...
    /* ==============================================================
        questão 3: busca sequencial no arquivo para o atributo chave;
       ============================================================== */
    ...
    /* ============================================================
       questão 4: busca do tipo >, >=, < ou <= usando a árvore binária de pesquisa ou AVL para um atributo não-chave.
    ...
    /* ===========================================================================================================
       questão 5: Busca do tipo >, >=, < ou <= usando busca sequencial no arquivo para o mesmo atributo do item 4.
       =========================================================================================================== */
